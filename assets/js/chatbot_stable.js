...
window.sendChatMessage = async () => {
    ...
    if (!apiKey) {
        appendMessage('system', '⚠️ API Key Not Found. Please check settings.');
...
function appendMessage(sender, text) {
    ...
    const html = `
        <div class="flex flex-col ${alignClass} fade-in-up">
            ...
            <span class="text-[10px] text-gray-400 mt-1 mx-1">${isUser ? 'You' : (isSystem ? 'System' : 'Nex')}</span>
        </div>
    `;
...
async function getPromotionsContext() {
    try {
        ...
        return hasPromos ? promoText : "Current Promotions: None.";
    } catch (e) {
        return "Promotions info unavailable.";
    }
}

async function getProductsContext() {
    try {
        const q = query(collection(db, "products"), limit(100)); // Safer limit
        const querySnapshot = await getDocs(q);
        if (querySnapshot.empty) return "Products: No items listed.";
...
=== REAL-TIME WEBSITE DATA (Store Inventory) ===
${promoContext}
...
Instructions:
1. Priority 1: Check Store Data.
2. Priority 2: Use Google Search (if available) for general questions.
3. Be correct, helpful, and concise.

Answer:`;

    const strategies = [
        { model: 'gemini-1.5-flash', version: 'v1beta', useTools: true }, // Try Search
        { model: 'gemini-1.5-flash', version: 'v1beta', useTools: false }, // Fallback Flash
        { model: 'gemini-1.5-pro', version: 'v1beta', useTools: false }   // Fallback Pro
    ];
    ...
    for (const strat of strategies) {
        try {
            const url = `https://generativelanguage.googleapis.com/${strat.version}/models/${strat.model}:generateContent?key=${key}`;
            const requestBody = { contents: [{ parts: [{ text: fullPrompt }] }] };
            if (strat.useTools) requestBody.tools = [{ googleSearchRetrieval: {} }];
            ...
            console.warn(`Strategy Failed (${strat.model}, Tools: ${strat.useTools}):`, e);
            lastError = e;
        }
    }

    throw new Error(lastError ? lastError.message : "All AI models failed.");
}
